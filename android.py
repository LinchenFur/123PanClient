...
class Pan123:
    ...
    def login(self):
        ...
        try:
            ...
            print(f"响应状态码: {login_res.status_code}")
            
            login_res.raise_for_status()
            
            res_json = login_res.json()
            print(f"登录响应: {json.dumps(res_json, indent=2)}")
            
            # 正确的成功状态码是200，不是0
            if res_json.get("code") != 200:
                print(f"登录失败: {res_json.get('message', '未知错误')}")
        ...
        try:
            if os.path.exists(config_file):
                ...
                if os.path.exists(old_file):
                    ...
                    # 迁移到新配置文件
                    self.user_name = old_config["userName"]
                    self.password = old_config["passWord"]
                    self.authorization = old_config["authorization"]
                    self.save_file()  # 保存为新格式
                    print(f"已从{old_file}迁移配置到{config_file}")
                    return
                else:
